...
<template>
    <div class="petal-query-container">
        ...
        <DataTable :columns="columns" :data="petalStore.items" :total="petalStore.pagination.total"
            :page-size="petalStore.pagination.limit" :loading="petalStore.loading" :showActions=false
            @page-change="handlePageChange" @size-change="handleSizeChange" />

        <!-- 错误提示 -->
        <el-alert v-if="petalStore.error" :title="petalStore.error" type="error" show-icon class="mt-4" />
    </div>
</template>
