...
<template>
    <el-select v-model="selectedIds" multiple filterable remote reserve-keyword placeholder="搜索并选择物种"
        :remote-method="searchSpecies" :loading="loading" @change="handleChange">
        <el-option v-for="item in speciesOptions" :key="item.species_id" :label="itemLabel(item)"
            :value="item.species_id" />
        <el-pagination small layout="prev, pager, next" :total="pagination.total" :page-size="pagination.pageSize"
